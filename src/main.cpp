...
#if defined(NDEBUG)
# error "Azercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7abd3db070a99c0303b656b8bcc965a436be256bd497e0c06ebc05ff91025a37");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Azercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Azercoin Signed Message:\n";
...
{
    // Azercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Azercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Azercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1505222714, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Hurricane Irma downgraded to Category 2 storm with 110 mph winds";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047de26ace5926aa93c94ceb1427913db5c871718e87a8f95206e09feea67a605b5a295749c9ca7c0b7f5e505c33314e5a1c001d88cadf4626085e8fcc6abd8ebe") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1505222714;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1213269;
        ...
        assert(block.hashMerkleRoot == uint256("0x8eb8ef6651fb93f91807c29a2636ee31de9eaba97f0d54b3e4c91762f46899ab"));
...
unsigned char pchMessageStart[4] = { 0x35, 0x90, 0x8b, 0xf5 }; // Azercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AzercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AzercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AzercoinMiner(CWallet *pwallet)
{
    printf("AzercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("azercoin-miner");
    ...
    try { loop {
        ...
        printf("Running AzercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AzercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AzercoinMiner, pwallet));
}
